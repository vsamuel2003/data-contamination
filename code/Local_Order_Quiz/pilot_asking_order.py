...
def order(dataset, num_options, split, model):
    ...
    sampled_qs = data_loader(f'../datasets/{dataset}/{split}.txt')
    ...
    for instance in sampled_qs:
        ...
        meta = gen_p(qb, instance, dataset, num_options) 
        the_gt = meta['gt']
        example = meta['tar_text']
        ...
        res.append(inp)
        print(f'Done with {len(res)}/100 for dataset {dataset}')
